@if($errors->any())
    @foreach($errors->all() as $message)
        <div class="flex items-center p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400"
            ...
            <div><span class="font-medium">{!! $message !!}</span></div>
        </div>
    @endforeach
@endif
{{--For Info--}}
...
@endif
{{--For Danger--}}
@if(session('error') || session('error_message') || session('alert_switch'))
    <div class="flex items-center p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400"
         role="alert">
        <svg class="flex-shrink-0 inline w-4 h-4 me-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
             fill="currentColor" viewBox="0 0 20 20">
        ...
        <span class="sr-only">Info</span>
        <div>
            <span class="font-medium">{{session('alert_switch')}}</span>
        </div>
    </div>
@endif
{{--For Success--}}
...
@if(session("error_dark"))
    <div class="flex items-center p-4 text-sm text-gray-800 rounded-lg bg-gray-50 dark:bg-gray-800 dark:text-gray-300"
        ...
        <span class="sr-only">Info</span>
        <div>
            <span class="font-medium">{{ session("error_dark") }}</span>
        </div>
    </div>
@endif
